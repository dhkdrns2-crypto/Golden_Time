...
public class UserService {
    ...
    @Transactional
    public void addVehicle(String loginId, VehicleUpdateDto vehicleDto) {
        User user = userRepository.findByLoginId(loginId)
                .orElseThrow(() -> new IllegalArgumentException("사용자를 찾을 수 없습니다: " + loginId));
    ...
    public void updateRole(Long userId, String role) {
        ...
        if ("admin".equals(user.getLoginId())) {
            throw new IllegalStateException("기본 관리자 계정의 권한은 변경할 수 없습니다.");
        }

        String upperRole = role.toUpperCase();
        if (!upperRole.equals("ADMIN") && !upperRole.equals("USER")) {
            throw new IllegalArgumentException("유효하지 않은 권한입니다. (ADMIN 또는 USER만 가능)");
        }
