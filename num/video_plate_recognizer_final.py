...
# --- 한글 텍스트 표시 최적화 (GPU 연산 대신 캐시된 비트맵 사용) ---
_kr_font_cache = {}
_kr_text_cache = {}
...
def get_korean_text_overlay(text, color=(0, 255, 0), size=24):
    ...
    tw, th = font.getbbox(text)[2:] # 텍스트 크기 측정
...
class ANPR_Pro_System:
    def __init__(self):
        self.engine = PlateEnginePro()
        self.engine.config.DETECT_CONF = 0.45
        self.engine.config.OCR_CONF = 0.65
        self.engine.consecutive_required = 2
        
        self.detected_plates = defaultdict(list)
        self.frame_queue = queue.Queue(maxsize=1) # 최신 1프레임만 대기
        self.latest_results = []
        self.running = True
        self.frame_count = 0
        self.display_res = (1280, 720) # 출력 해상도 고정 (속도 향상 핵심)

    def worker_thread(self):
        """인식 전용 스레드 (메인 루프와 완전 분리)"""
        while self.running:
            try:
                ...
                for r in results:
                    ...
                    if len(self.detected_plates[p]) < 100:
                        self.detected_plates[p].append(r['confidence'])
    ...
    def run(self, video_path):
        ...
        cv2.namedWindow("ANPR 60FPS Pro", cv2.WINDOW_AUTOSIZE)
        ...
        try:
            while True:
                ...
                for r in self.latest_results:
                    ...
                    draw_korean_text_fast(display_frame, f"{p} ({c:.2f})", (bx[0], bx[1]-30), size=24)

                # 정보창
                curr_fps = self.frame_count / (time.time() - start_time)
                info = f"FPS: {curr_fps:.1f} | Detected: {len(self.detected_plates)}"
                cv2.rectangle(display_frame, (5, 5), (280, 45), (0,0,0), -1)
                draw_korean_text_fast(display_frame, info, (10, 10), (255,255,255), 20)
                
                cv2.imshow("ANPR 60FPS Pro", display_frame)
    ...
    def summary(self):
        print("\n" + "="*50 + "\n  인식 결과 요약 (100회 제한)\n" + "="*50)
        items = sorted([(p, np.mean(c), len(c)) for p, c in self.detected_plates.items() if len(c) >= 2], 
                       key=lambda x: x[1], reverse=True)
        print(f"{'번호판':<15} | {'평균정확도':<10} | {'인식횟수':<10}")
        for p, a, c in items: print(f"{p:<15} | {a:.2%} | {c:<10}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("video")
    args = parser.parse_args()
    ANPR_Pro_System().run(args.video)
